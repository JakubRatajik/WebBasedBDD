...
let mouseX = 0;
let mouseY = 0;
let showOnlyScenarios = false;
let showOnlyEntities = false;

function showSavedBlocks() {
	let localStorageName, workspace, checkBoxText, expectedBlockType;
	
	const container = document.getElementById('saved-blocks-container');
	...
	if (currentTab === entitiesTab){
		...
		checkBoxText = "Show only entity blocks.";
		expectedBlockType = "DeclarativeEntityDef";
		checkbox.checked = showOnlyEntities;
		...
		localStorageName = 'savedScenarioBlocks';
		workspace = scenarioWorkspace;
		checkBoxText = "Show only scenario blocks."
		expectedBlockType = "Scenario";
		checkbox.checked = showOnlyScenarios;
		checkbox.addEventListener('change', function () {
		    if (this.checked) {
		    	showOnlyScenarios = true;
		    } else {
	...
	const savedBlocks = JSON.parse(localStorage.getItem(localStorageName) || '[]');	
    container.innerHTML = '';
	

	
	
	const labelText = document.createTextNode(checkBoxText);
    ...
    if (savedBlocks.length === 0) {
        ...
        emptyMessage.textContent = 'No blocks saved yet!';
        emptyMessage.style.textAlign = 'center';
        container.appendChild(emptyMessage);
    } else {
		const filteredBlocks = checkbox.checked
		        ? savedBlocks.filter(block => block.type === expectedBlockType)
		        : savedBlocks;
		
        filteredBlocks.forEach((blockJson) => {
            const blockPreviewWrapper = document.createElement('div');
			blockPreviewWrapper.className = 'block-preview-wrapper';

            const blockPreviewDiv = document.createElement('div');
			blockPreviewDiv.className = 'block-preview-div';
            blockPreviewWrapper.appendChild(blockPreviewDiv);

			const deleteButton = document.createElement('button');
			deleteButton.className = 'delete-library-block-button';
			deleteButton.textContent = '×';
			deleteButton.onclick = (e) => {
			    e.stopPropagation();
				const blockIndex = savedBlocks.findIndex(
				                    block => JSON.stringify(block) === JSON.stringify(blockJson)
				                );
			    //filteredBlocks.splice(index, 1);
//			    localStorage.setItem(localStorageName, JSON.stringify(filteredBlocks));
//				container.style.display = 'none';
//			    showSavedBlocks();
				if (blockIndex !== -1) {
                    savedBlocks.splice(blockIndex, 1);
                    localStorage.setItem(localStorageName, JSON.stringify(savedBlocks));
                    showSavedBlocks();
                }
			};			
            blockPreviewWrapper.appendChild(deleteButton);

            const miniWorkspace = Blockly.inject(blockPreviewDiv, {
                readOnly: true,
                zoom: { controls: false, wheelwork: false },
            });

            Blockly.serialization.blocks.append(blockJson, miniWorkspace);

            blockPreviewWrapper.onclick = () => {
                Blockly.serialization.blocks.append(blockJson, workspace);
            };

            container.appendChild(blockPreviewWrapper);
        });
    }
...
function setSelectionBorder(element) {
	element.style.backgroundColor = "89a8b2";
}

function removeSelectionBorder(element) {
	element.style.backgroundColor = "b3c8cf";
}

function setDisabled(element) {
	element.style.backgroundColor = "#b7b7b7";
...
function hoversMouseOverLibrary() {
	const bookIcon = document.getElementById('book-icon');
	const bookIconRect = bookIcon.getBoundingClientRect();
	
  	return (
	    mouseX >= bookIconRect.left &&
	    mouseX <= bookIconRect.right &&
	    mouseY >= bookIconRect.top &&
	    mouseY <= bookIconRect.bottom
	);
}
...
function storeBlockToLibrary(block) {
    ...
    let savedBlocks = JSON.parse(localStorage.getItem(localStorageName) || '[]');
...
function loadBlocks(element, skipAddingBlocks) {
	fetch('/xtext-service/blocks?resource=multi-resource/scenarios.bdd')
		...
		.then(response => {
			...
			document.getElementById('book-icon').addEventListener('click', showSavedBlocks);
			
			document.addEventListener('click', (event) => {
			    const container = document.getElementById('saved-blocks-container');
			    const bookIcon = document.getElementById('book-icon');
			    if (!container.contains(event.target) && !bookIcon.contains(event.target)) {
			        container.style.display = 'none';
			    }
			});
			...
			if (element === scenarioTab && !scenarioToolboxInjected)
			{
				...
				scenarioWorkspace.addChangeListener(dragAndDropToLib)
			...
			if (element === entitiesTab && !entitiesToolboxInjected)
			{
				...
				entityWorkspace.addChangeListener(dragAndDropToLib)
			...
			function dragAndDropToLib(event) {
			  if (event.type === Blockly.Events.BLOCK_DRAG && !event.isStart) {
				let currentWorkspace;
				if (currentTab === entitiesTab){
					currentWorkspace = entityWorkspace;
				}
				else {
					currentWorkspace = scenarioWorkspace;
				}
			    ...
			    if (hoversMouseOverLibrary()) {
					storeBlockToLibrary(block);
			    }
			  }
			}
...
function saveScenario() {
  const scenarioContent = document.getElementById("xtext-editor-scenarios").env.document.doc.getValue();
...
function saveEntities() {
  const entityEditorContent = document.getElementById("xtext-editor-entities").env.document.doc.getValue();
  ...
  fetch('/save-entities', {
    ...
    body: JSON.stringify({ content: entityEditorContent }), // Send the scenario content as JSON
  }).then(response => {
    if (response.ok) {
